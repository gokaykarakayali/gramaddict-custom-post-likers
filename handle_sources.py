...
def handle_likers(
    ...
    if current_job == "blogger-post-likers":
        if not nav_to_post_likers(device, target, session_state.my_username):
            return False
    elif current_job != "custom-post-likers":  # custom-post-likers için navigasyon atlanır
        if not nav_to_hashtag_or_place(device, target, current_job):
            return False

    # For custom-post-likers, we are already in the likers list
    if current_job == "custom-post-likers":
        # Wait for the likers list to load
        likes_list_view = OpenedPostView(device)._getListViewLikers()
        if likes_list_view is None:
            logger.error("Likers list didn't load for custom post!")
            return
    ...
    # Now proceed to iterate likers
    while True:
        logger.info("Iterate over visible likers.")
        screen_iterated_likers = []
        screen_skipped_followers_count = 0  # Değişkeni burada tanımlıyoruz
        opened = False
